...
function ItemSummary({ item }: { item: ConfiguredItem }) {
  const { config } = item;
  const lines: string[] = [];

  if (config.contentType === "cuaderno" && config.contentSubtype) {
    lines.push(`Cuaderno ${config.contentSubtype}`);
  } else {
  ...
  return (
    <div className="space-y-0.5">
      {lines.map((l) => (
        <p key={l} className="font-sans text-xs text-warmgray">{l}</p>
      ))}
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center gap-4 text-center">
                  ...
                  <p className="font-sans text-sm text-warmgray/60">Elige una portada y configura tu cuaderno.</p>
                ...
                items.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="w-20 h-24 rounded-sm overflow-hidden bg-cream-warm flex-shrink-0">
                      <Image
                        src={item.config.cover.images[0] ?? "/images/covers/placeholder.jpg"}
                        alt={item.config.cover.name}
                        width={80}
                        height={96}
                        unoptimized
